...
export const kanbanItemCommentsQueryOptions = (itemId: string) =>
  queryOptions({
    ...
    queryFn: () => getKanbanItemCommentsFn({ data: { itemId }, headers: getAuthHeaders() }),
  });
...
export const kanbanItemCommentRepliesQueryOptions = (parentCommentId: string) =>
  queryOptions({
    ...
    queryFn: () => getKanbanItemCommentRepliesFn({ data: { parentCommentId }, headers: getAuthHeaders() }),
  });

export const kanbanItemCommentCountQueryOptions = (itemId: string) =>
  queryOptions({
    queryKey: ["kanban-item-comment-count", itemId],
    queryFn: () => countKanbanItemCommentsFn({ data: { itemId }, headers: getAuthHeaders() }),
  });
