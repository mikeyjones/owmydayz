...
import { getHeartCountFn, getHeartStatusFn } from "~/fn/hearts";
...
export const getHeartStatusQuery = (songId: string) =>
	queryOptions({
		...
		queryFn: () =>
			getHeartStatusFn({ data: { songId }, headers: getAuthHeaders() }),
	});
...
export const getHeartCountQuery = (songId: string) =>
	queryOptions({
		...
		queryFn: () =>
			getHeartCountFn({ data: { songId }, headers: getAuthHeaders() }),
	});
