...
export const getPostReactionStatusQuery = (postId: string) =>
	queryOptions({
		...
		queryFn: () =>
			getPostReactionStatusFn({ data: { postId }, headers: getAuthHeaders() }),
	});
...
export const getPostLikeCountQuery = (postId: string) =>
	queryOptions({
		...
		queryFn: () =>
			getPostLikeCountFn({ data: { postId }, headers: getAuthHeaders() }),
	});
...
export const getCommentLikeCountQuery = (commentId: string) =>
	queryOptions({
		...
		queryFn: () =>
			getCommentLikeCountFn({ data: { commentId }, headers: getAuthHeaders() }),
	});
